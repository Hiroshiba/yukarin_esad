...
Diffusionモデルベースの生成方式を採用：
- Rectified FlowモデルとMean Flowモデルの2種類の手法を実装
- 音素ID列と母音/ストレスフラグを組み合わせた条件情報を使用
- ノイズから徐々にF0軌跡を生成する反復的な推論プロセス
- 母音フラグマスクを適用して母音部分のF0のみを生成対象とする
- F0と有声/無声を同時に生成
...
- Diffusionモデルベースのネットワーク構造
- Rectified FlowとMean Flowの2種類の手法に対応
- 固定長・可変長データの統一処理
- 音素エンベディング後のLinear層4層による特徴変換
- F0と有声/無声の同時生成

#### Diffusionモデルアーキテクチャ（予定）
- **入力**: 音素ID列 + 母音/ストレスフラグ列（全音素数の長さ） + ノイズ付きF0
- **エンコーダ**: Conformerエンコーダによる文脈特徴抽出
- **サンプリング**: Rectified FlowまたはMean Flowによる反復的な生成プロセス
